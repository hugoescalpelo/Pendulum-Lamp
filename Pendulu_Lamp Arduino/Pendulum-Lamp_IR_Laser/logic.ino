...
void add ()
{
  for (byte i_r = 0; i_r < LENGTHC1 - 1; i_r++)
  {
...
void extractDetect ()
{
  for (byte i_c = 0; i_c < NSENSORS; i_c++)
  {
    ...
    if (maxR != 0 || minR != 1023)
    {
      ...
      if (c1 [i_c][LENGTHC1 - 1] < maxR - dinamicThreshold [i_c])
      {
        addDetect (i_c, c1 [i_c][LENGTHC1 - 1]);
      }
    }
    byte c = 0;
    int b = 0;
    for (int i_r = 0; i_r < LENGTHC1; i_r++)
    {
...
{
  for (byte i_r_ad = 0; i_r_ad < LENGTHC1 - 1; i_r_ad++)
  {
    int shiftAux_ad = c2 [i_c_ad][i_r_ad + 1];
    c2 [i_c_ad][i_r_ad] = shiftAux_ad;
  }
  c2 [i_c_ad][LENGTHC1 - 1] = minRAD;
}
